...
public class CustomersMessageBodyWriterV1 implements MessageBodyWriter<List<Customer>> {
    ...
    public void writeTo(
        ...
        MessageBodyWriter<List<CustomerV1>> jsonWriter
            = (MessageBodyWriter<List<CustomerV1>>)(MessageBodyWriter<?>)providers.getMessageBodyWriter(
                    List.class,
                    new GenericType<List<CustomerV1>>() {}.getType(),
                    annotations,
                    MediaType.APPLICATION_JSON_TYPE);
        jsonWriter.writeTo(
                customers.stream().map(CustomerV1::new).collect(toList()),
                List.class,
                new GenericType<List<CustomerV1>>() {}.getType(),
                annotations,
                MediaType.APPLICATION_JSON_TYPE,
